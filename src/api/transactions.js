...
export async function getTransactionList({type, addresses}) {
    ...
    if (type === "BTC") {
        ...
        const receiverAddresses = receiver.map(i => i.transaction.hash);

        const data = [...receiver, ...sender].filter(item => item.currency.symbol === type);
        const sortByDate = data.sort((a, b) => {
            return new Date(`${b?.block?.timestamp?.time?.split(' ')[0]}T${b?.block?.timestamp?.time?.split(' ')[1]}`) - new Date(`${a?.block?.timestamp?.time?.split(' ')[0]}T${a?.block?.timestamp?.time?.split(' ')[1]}`)
        })
...
export async function getTransactionDetails(transaction) {
