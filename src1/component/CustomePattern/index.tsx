...
export default class GesturePassword extends Component {
  ...
  getCrossChar(char: any) {
    let middles = "13457",
      last = String(this.lastIndex)

    if (
      middles.indexOf(char) > -1 ||
      middles.indexOf(last) > -1
    )
      return false
    ...
    for (let i = 0; i < middles.length; i++) {
      let index = middles[i]
      if (
        helper.isEquals(point, this.state.circles[index])
      ) {
        return String(index)
      }
    }
  ...
  onMove = (e: any, g: any) => {
    if (this.isMoving) {
      ...
      if (
        lastChar &&
        this.sequence.indexOf(lastChar) === -1
      ) {
        if (!this.props.allowCross) {
          ...
          if (
            crossChar &&
            this.sequence.indexOf(crossChar) === -1
          ) {
...
const Lines = memo(
  ({
    ...
    return lines.map(function (l: any, i: any) {
...
const Circles = memo(
  ({
    ...
    return circles.map(function (c: any, i: any) {
      fill = c.isActive
      color = status === "wrong" ? wrongColor : rightColor
      inner = !!innerCircle
      outer = !!outerCircle
