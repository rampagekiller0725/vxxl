...
export function getTransaction(type: any) {
  const Transaction = bitcore.Transaction;
  const Input = Transaction.Input;
  const Output = Transaction.Output;

  const checkArgument = function (condition: any, argumentName: any, message?: any, docsPath?: any) {
    if (!condition) {
  ...
  if (type === 'vxxl') {
    Transaction.prototype.fromBufferReader = function (reader: any) {
  ...
  if (type === 'vxxl') {
    Transaction.prototype.toBufferWriter = function (writer: any) {
    ...
    Transaction.prototype.toBufferWriter = function (writer: any, noWitness: any) {
